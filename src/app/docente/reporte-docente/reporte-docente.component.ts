...
export class ReporteDocenteComponent implements OnInit {
  ...
  calcularPromedioFinal(calificacion: CalificacionSecundariaDTO): string {
    let promedio1 =
      (calificacion.primeraEvaluacion + calificacion.segundaEvaluacion) / 2;
    let promedio2 =
      (calificacion.terceraEvaluacion + calificacion.cuartaEvaluacion) / 2;

    return (
      (promedio1 + promedio2 + calificacion.quintaEvaluacion) /
      3
    ).toFixed(1);
  }
  ...
  convertirArreglo() {
    this.calificacionesDocente.calificaciones.forEach((element) => {
      ...
      let final = (promedio1 + promedio2 + element.quintaEvaluacion) / 3;
      ...
      arrTemp.push(element.quintaEvaluacion);
