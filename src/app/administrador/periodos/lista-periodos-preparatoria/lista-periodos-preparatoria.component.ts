...
@Component({
  selector: 'app-lista-periodos-preparatoria',
  templateUrl: './lista-periodos-preparatoria.component.html',
  styleUrls: ['./lista-periodos-preparatoria.component.css']
})
export class ListaPeriodosPreparatoriaComponent implements OnInit {
  ...
  obtenerPeriodosPaginacion(pagina: number, cantidadRegistrosAMostrar: number) {
    ...
    this.periodosService
      .preparatoriaPaginacion(pagina, cantidadRegistrosAMostrar)
      .subscribe({
  ...
  filtrarPeriodos(values: any) {
    ...
    this.periodosService.filtrarPreparatoria(values).subscribe({
      next: (response) => {
        this.isLoading = false;
